...
const scriptsInEvents = {
	...
	async EventSheetMain_Event36_Act1(runtime, localVars)
	{
	...
	async EventSheetMain_Event38_Act1(runtime, localVars)
	{
		const player = runtime.objects.Player.getFirstInstance();
		if (!player) return;
		...
		const bullet = runtime.objects.DroneBullet.getFirstPickedInstance();
		if (bullet) bullet.destroy();
	},

	async EventSheetMain_Event14_Act2(runtime, localVars)
	{
		const player = runtime.objects.Player.getFirstInstance();
		if (!player) return;
